...
pointseny_list=[points_eny_2,points_eny_4,points_eny_40,points_eny_100,points_eny_200]

pointsiny_list=[points_iny_2,points_iny_4,points_iny_40,points_iny_100,points_iny_200]

pointsyong_list=[points_yong_2,points_yong_4,points_yong_40,points_yong_100,points_yong_200]
...
col=['black','orangered','green','royalblue','fuchsia','orange','darkcyan']



test_en_list = [curve_fit(en_fit,pointsyong_enx,v,sigma=0.1*v)[0]for v in pointsyong_list]#fit de en avec les donnees de Yong

#test_en_list = [curve_fit(en_fit,pointsyong_enx,v,sigma=0.01)[0]for v in pointsyong_list]

test_in_list = [curve_fit(in_fit,points_inx,v,sigma=0.1*v)[0]for v in pointsiny_list]#fit de in avec les donnees de Yong
...
for v,capa,yong,colo in zip(test_en_list,capa_list,pointsyong_list,col):
    ...
    plt.loglog(t_range_full, en_fit(t_range_full,*v),color=colo, linestyle='-'
                               ,label='Hemt C='+capa+' e0={0:.2e}, ea={1:.2e}'.format(*v))
    
    plt.plot(pointsyong_enx, yong,linestyle='',color=colo,marker='o',markersize=5,label='Donnees en table 1 Hemt de '+capa)
    
    #plt.loglog(t_range_full, en_fit(t_range_full,*para),color=colo, linestyle=':'
    #                           ,label='Fit_en Alex Hemt C='+capa+' e0={0:.3e} V, ea={1:.3e} V'.format(*para))
...
for v,capa,iny,colo in zip(test_in_list,capa_list,pointsiny_list,col):
    ...
    v=abs(v)
    plt.loglog(t_range_full, in_fit(t_range_full,*v),  linestyle='-',color=colo
                               ,label='Hemt C='+capa+'  i0={1:.2e}, ia={0:.2e}'.format(*v))
    
    plt.plot(points_inx, iny,color=colo,linestyle='',marker='o',markersize=5,label='Donnees in table 1 Hemt de '+capa)
    
    #plt.loglog(t_range_full, in_fit(t_range_full,*para),color=colo, linestyle=':'
    #                           ,label='Fit_in  Alex Hemt C='+capa+' ia={0:.3e}'.format(*para))
