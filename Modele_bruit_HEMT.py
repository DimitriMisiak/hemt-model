...
class FET:
    ...
    def __init__(self,e0_ini,ea_ini,eb_ini,i0_ini,ia_ini,ib_ini,Cfet_ini):
        self.e0=e0_ini*1e-9
        self.ea=ea_ini*1e-9
        self.eb=eb_ini*1e-9
        self.i0=i0_ini*1e-18
    ...
    def en1(self,freq):
        self.en=np.sqrt(self.e0**2+self.ea**2/freq+(self.eb/freq)**2)
        return self.en
...
def fig1():
    ...
    for v in [t2]:
         #while i2<np.size(np.array(R[0][:])):
             for l in range(1,int(np.size(fs))):
                 ...
                 entot[l-1]=np.sqrt(en[l-1]**2+(in_[l-1]*impedance[l-1])**2+bjohnson[l-1])
                 ...
                 if l in [1,1000]:
                     ...
                     print(zae)
                     print(str(l)+'Hz Z='+str(impedance[l-1]*1e-6)+'MO   '+str(v.Chemt*1e12),' in=',str(a))
                 '''
             
             pl.loglog(range(int(np.size(fs))), entot, linestyle='-',linewidth=2
                       ,color=col[tes],label='Hemt pour R='+str(R*1e-6)+'MO C='+str(v.Chemt*1e12)+'pF')
             
             
             pl.loglog(range(int(np.size(fs))), in_, linestyle=':'
                       ,color=col[tes],label='in*Z Hemt R='+str(R*1e-6)+'MO C='+str(v.Chemt*1e12)+'pF')
             
             pl.loglog(range(int(np.size(fs))), en, linestyle='--'
                       ,color=col[tes],label='en Hemt R='+str(R*1e-6)+'MO C='+str(v.Chemt*1e12)+'pF')
    ...
    for l in range(1,int(np.size(fs))):
        ...
        in_[l-1]*=impedance[l-1]    
        # i2+=2
    pl.loglog(range(int(np.size(fs))), entot, linewidth=3,linestyle=':',color=col[tes]
                   ,label='Fet pour R='+str(R*1e-6)+'MO C='+str(v.Cfet*1e12)+'pF')
